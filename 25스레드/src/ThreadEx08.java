...
public class ThreadEx08 {
	...
	//주스레드를 기준으로해서 사용자정의 스레드 그룹 직접 만들기
	public static void main(String[] args) {
		...
		//grp1 스레드 그룹에 subGrp1 스레드그룹을 만들어서 포함시키자
		ThreadGroup subGrp1 = new ThreadGroup(grp1, "SubGroup1");
							//new ThreadGroup(부모스레드그룹객체의주소, "생성하는스레드그룹명");
		
		//어떤 특정 스레드를 grp1 스레드 그룹에 가입시켜야 합니다.
		//가입시키기전에 grp1 스레드 그룹에 가입된 스레드 객체들의 우선 작업순위를
		//3으로 설정하자
		grp1.setMaxPriority(3);
		//우선순위란?
		//특정 스레드 그룹 안에 포함되어있는 스레드들이 더 많은 작업시간을 가지도록 한다
		//보충 설명) 현재 grp1스레드 그룹 안에 가입시킨 모든 스레들의 우선순위값을 
		//		   3으로 일괄적으로 지정한다.
		
		
		//grp1 참조변수가 사용하고 있는 grp1스레드그룹에 가입시킬 보조작업스레드 객체 생성 후 가입시키기
		Thread t1 = new Thread(grp1, "t1");
		
		//subGrp1 스레드 그룹에 포함(가입)시킬 보조작업스레드객체 생성 후 가입
		Thread t2 = new Thread(subGrp1, "t2");
		
		//grp2 스레드 그룹에 포함(가입)시킬 보조작업스레드객체 생성 후 가입
		Thread t3= new Thread(grp2, "t3");
		
		//보조작업스레드 객체 t1, t2, t3 일 시작 시키자
		t1.start(); t2.start(); t3.start();
		
		//현재 작업을 실행하고 있는(run 메소드의 코드가 실행되고 있는)
		//main 스레드가 속해 있는 그룹에 대한 스레드그룹 이름 얻기
		System.out.println(">> List of ThreadGroup : " + g_main.getName()); //main
		
		//현재 생성된 스레드 그룹의 갯수 출력
		System.out.println("Activity ThreadGroup : " + g_main.activeGroupCount()); //3
		//grp1, grp2, subGrp1
		
		//스레드 그룹에 관한 모든 정보를 자세히 출력해보자
		//참고.
		//이 스레드 그룹에 대한 정보를 표준 출력형식으로 받아 출력합니다.
		//이 메소드는 디버깅 테스트의 경우에만 사용합니다.
		g_main.list();
